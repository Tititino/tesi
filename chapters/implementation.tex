...
A boolean expression, in this context, are expressions made of Prolog variables and the constants 1 and 0, respectively true and false.
...
As a side effect, the flag mandates that all Prolog variables are wrapped by the functor \texttt{v/1}, so instead of writing
\begin{minted}{prolog}
X * Y =:= 1
\end{minted}
we have to write
...
For the prover we define a series of helper predicates ranging from ones that just implement $\phi\;\mathrm{asy}$ from Definition \ref{def:focusing predicates}, to ones which aid the generation of constraints.
