...
In 2001 D. Pym and J. Harland publish a paper \cite{HarlandPym} where they propose a new way to tackle the problem of splitting sequents during linear logic proof search using boolean expressions associated to each formula.
During proof search these expressions are then used to generate constraint that enforce linearity, and a proof is valid if its constraints are satisfiable.
...
Under these rules a certain judgment it true if and only if all of its formulae get used exactly once; for this reason this logic is sometimes called a logic of resources, in the same way classical logic is a logic of truths and intuitionistic logic is a logic of proofs.
...
In linear logic each connective of classical logic has two versions: an additive one -- where the two branches keep the same context, and a multiplicative one -- where the context gets portioned between the two branches.
To better understand why let's analyze classic conjunction, this can be defined as 
$$
\begin{array}{cc}
...
\end{array}
$$
These two rules are equivalent only if the use of weakening and contraction is permitted, so linear logic distinguishes the two interpretations: the left one is the additive one, and the right one the multiplicative one.
...
\begin{center}
	...
	\end{tblr}
\end{center}
It is the multiplicative side which brings the most complexity: in top-down proof search, the action of searching the correct partition of the sequents -- called splitting -- may imply an exponential number of attempts.
