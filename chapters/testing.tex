...
\end{minted}
The innermost decorator (line 3) accepts a dictionary, and is needed to automatically call the prover with a test entry (as in Section \ref{sec:file format}), time it and eventually terminate the process it if the time runs out.
...
\begin{tblr}{ colspec = {l l}
	...
	\textbf{conclusions} & a list of conclusions as prefix formulae. \\
\end{tblr}
\end{center}
For example
\begin{minted}{json}
...
\end{minted}
is the test case representing $\llnot{(a \llten b)} \lolli \llnot{a} \llpar \llnot{b}$.
...
\begin{tblr}{ colspec = {l l}
	...
	\textbf{thm} & whether this test case is a tautology or not, may be null; \\
	\textbf{*, +, ?, ...} &the number of times a specific connective appears in the test case; \\
	\textbf{notes} & human readable text about the test case, e.g. its infix representation; \\
	\textbf{size} & an indicative number of the size of the formula; \\
	\textbf{atoms} & the upper bound on the number of atoms.
\end{tblr}
\end{center}
...
These translations introduce a high number of exponentials; this causes -- apart from the timeouts because of the added complexity -- some failures.
These failures are not due to bugs, but instead happen because the prover has reached its bound on contractions for the proof.
The benchmarks are done using a timeout of \qty{60}{\second} and a bound of 3.
It can be seen in Table \ref{table:KLE-cbn} and \ref{table:KLE-cbv} that in this case our prover performs slightly worse than APLL.
