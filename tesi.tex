...
\begin{figure}[H]
	...
	\begin{tblr}{ colspec = {cccccccccr}
		...
		\phi & ::=  & 1              &\mid& \phi \llten \phi  &\mid& \bot &\mid& \phi \llpar \phi  & \text{(Multiplicatives and their constants)} \\
		     ...
		     & \mid & \llnot{\alpha} &\mid& \alpha	      &    &      &    &                   & \text{($\alpha$ a term)}
	\end{tblr}
...
\begin{figure}[H]
	...
	\begin{subfigure}{\textwidth}
		...
		\begin{tblr}{colspec = { cc }, rows = {abovesep=10pt, belowsep=10pt}}
			\SetCell[c=2]{c} {\small
			\AxiomC{$\focus{\constr{\used{e}, \Lambda}{V'}}{\Psi}{\text{split}(\Delta)_L}{\text{af}(\phi_1, e)}$}
			\AxiomC{$\focus{\constr{V'}{V''}}{\Psi}{\text{split}(\Delta)_R}{\text{af}(\phi_2, e)}$}
			\LeftLabel{$[\llten]$}
			...
			\\
			{\small
			\AxiomC{$\sat{ \used{e_1}, \avail{\Delta}, \Lambda}{V}$}
			\LeftLabel{$[1]$}
			\UnaryInfC{$\focus{\constr{\Lambda}{V}}{\Psi}{\Delta}{\af{\llone}{e_1}}$}
			\DisplayProof} 
			&
			{\small
			\AxiomC{$\focus{\constr{\used{e_1}, \avail{\Delta}, \Lambda}{V}}{\Psi}{\Delta}{\af{\phi}{e_1}}$}
			\LeftLabel{$[\,!\,]$}
	...
	\begin{subfigure}{\textwidth}
		...
		\begin{tblr}{ colspec = { c }
			...
			{\small
			\AxiomC{$ \sat{\used{e_1}, \used{e_2}, \avail{\Delta}, \Lambda}{V}$}
			\LeftLabel{$[I_1]$}
