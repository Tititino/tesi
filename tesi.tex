...
Setting the sistem to classical linear logic, simply adds some rules to the rules for intuitionistic linear logic.
...
This corresponds to the rule \texttt{to\_delta} which implements the rule $R\!\Uparrow$
\begin{lstlisting}[language=prolog]
async(A, U, D, [F|Fs], S, M, In, _) :-
  F = af((Op-_), _, _),
  not(is_asy(Op)), !,
