...
\section{Why Prolog}
Prolog come linguaggio e come ambiente è sempre stato legato all'albito dell'automated theorem proving per la sua abilità di esprimere di gestire in modo naturale il backtracking; a differenza della maggior parte degli altri linguaggi
...
This allows to have constraints referencing some attributes of variables in the body of clauses, in our case we use CLP($\mathcal{B}$) \cite{clpb} for boolean constraints and an handy interface to a sat-solver.
...
Usually constraints will be accumualted in a list, for this reason CLP($\mathcal{B}$) provides the functor \texttt{*(L)} to express the conjunction of its members.
...
\begin{define}[New variables]
	...
	\begin{itemize}
		\item the variable name $x$ has not yet occurred in any expression in the proof tree, i.e. does not appear in any constraint of the father, of its (the father) siblings and their subtrees.
		\item each variable name $x_i, x_j \in X$ has not yet occurred in the proof and 
			$$ \forall i, j \mid i \neq j \Rightarrow x_i \neq x_j $$
...
\begin{define}[Constraints]
	...
	\begin{itemize}
		\item ``$\used{e}$'' states that the formula $\phi$ gets consumed in this branch of the proof.
			This corresponds to saying the constraint $e$ is true or
		...
		\item ``$\avail{e}$'' states that the formula $\phi$ does not get consumed in this branch of the proof, and thus is available to be used in another branch.
			This corresponds to saying the constraint $e$ is not true or
...
\begin{define}[Members of the sequent]
	...
	\begin{itemize}
		...
		\item $\Lambda$ and V are the constraints are solutions as defined in Definition \ref{def:sat}.
...
\begin{define}[Splitting]
	...
	\noindent With a slight abuse of notation we will write $\text{split}(\Delta, X)_L$ and $\text{split}(\Delta, X)_R$ respectively as the left projection and the right projection of the pair $(\Delta_L, \Delta_R)$.
\end{define}
...
this is split into
\begin{align*}
	\text{split}(\Delta, X)_L &\mapsto \af{a \llten b}{x_3 \varAnd x_1}, \af{\llnot{c}}{x_4 \varAnd x_2} \\
	\text{split}(\Delta, X)_R &\mapsto \af{a \llten b}{\varNot{x_3} \varAnd x_1}, \af{\llnot{c}}{\varNot{x_4} \varAnd x_2} 
\end{align*}
...
\begin{figure}[H]
	\begin{subfigure}{\textwidth}
		\centering
			...
			\AxiomC{}
			\LeftLabel{$[\lltop]$}
			\UnaryInfC{$\async{\Psi}{\Delta}{\af{\lltop}{-}, \Phi} \separator \constr{\Lambda}{\text{V}}$}
			\DisplayProof
			}
			\\
			\SetCell[c=2]{c} {\scriptsize
			...
			\LeftLabel{$[\llwith]$}
			\BinaryInfC{$\async{\Psi}{\Delta}{\af{\phi_1 \llwith \phi_2}{e}, \Phi} \separator \constr{\Lambda}{\text{V}', \text{V}''}$}	% capisco cosa fanno qui i constraint
			\DisplayProof}
			\\
			\SetCell[c=2]{c} {\scriptsize
			...
			\SetCell[c=2]{c} {\scriptsize
			\AxiomC{$\neg\isAsy{\phi}$}
			\AxiomC{$\async{\Psi}{\Delta, \af{\phi}{e}}{\Phi} \separator \constr{\Lambda}{\text{V}}$}
			\LeftLabel{$[R\!\Uparrow]$}
			\BinaryInfC{$\async{\Psi}{\Delta}{\af{\phi}{e}, \Phi} \separator \constr{\Lambda}{\text{V}}$}
			\DisplayProof
			}
		\end{tblr}
...
\begin{figure}[H]
	...
	\begin{subfigure}{\textwidth}
		...
		\begin{tblr}{ colspec = { cc } 
			...
			\SetCell[c=2]{c} {\scriptsize
			\AxiomC{$ \new{X} \hspace{.3cm} 
				\focus{\Psi}{\text{split}(\Delta, X)_L}{\text{af}(\phi_1, e)} \separator \constr{\Lambda, \used{e}}{\text{V}'} \hspace{.3cm} 
				\focus{\Psi}{\text{split}(\Delta, X)_R}{\text{af}(\phi_2, e)} \separator \constr{\text{V}'}{\text{V}''}$}
			\LeftLabel{$[\llten]$}
			\UnaryInfC{$\focus{\Psi}{\Delta}{\text{af}(\phi_1 \llten \phi_2, e)} \separator \constr{\Lambda}{\text{V}''}$}	% capisco il movimento dei constraint
			\DisplayProof}
			\\ 
			{\scriptsize
			...
			\AxiomC{$\async{\Psi}{\Delta}{\af{\phi}{e}} \separator \constr{\Lambda}{\text{V}}$}
			\LeftLabel{$[R\!\Downarrow]$}
			\BinaryInfC{$\focus{\Psi}{\Delta}{\af{\phi}{e}} \separator \constr{\Lambda}{\text{V}}$}
			\DisplayProof
			}
		\end{tblr}
...
\begin{figure}[H]
	...
	\begin{subfigure}{\textwidth}
		...
		\begin{tblr}{ colspec = { cc }
			...
			{\scriptsize
			\AxiomC{$\isPosLit{\phi}$}
			\AxiomC{$\focus{\Psi}{\Delta}{\af{\phi}{e}} \separator \constr{\Lambda}{\text{V}}$}
			\LeftLabel{$[D_1]$}
			\BinaryInfC{$\async{\Psi}{\Delta, \af{\phi}{e}}{.} \separator \constr{\Lambda}{\text{V}}$}
			\DisplayProof}
			\\
			{\scriptsize
			\AxiomC{$ \sat{\Lambda, \used{e_1}, \avail{\Delta}}{\text{V}}$}
			...
			\AxiomC{$\isPosLit{\phi}$}
			\AxiomC{$\new{x}$}
			\AxiomC{$\focus{\Psi}{\Delta}{\af{\phi}{x}} \separator \constr{\Lambda, \used{e}}{\text{V}}$}
			\LeftLabel{$[D_2]$}
...
\end{figure}
\begin{teor}
	The calculus of Figure \ref{fig:calculus} is sound % ...
\end{teor}
\begin{proof}
\end{proof}
