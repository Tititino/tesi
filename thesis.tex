...
\begin{figure}[H]
	\begin{subfigure}{\textwidth}
		\centering
			...
			\AXC{}
			\LeftLabel{$[\lltop]$}
			\UIC{$\async{\Psi}{\Delta}{\af{\lltop}{-}, \Phi} \separator \constr{\Lambda}{\text{V}}$}
			\DP
			}
			\\
			\SetCell[c=2]{c} {\footnotesize
			\AXC{$\async{\Psi}{\Delta}{\af{\phi_2}{e}, \Phi} \separator \constr{\Lambda, \used{e}}{\text{V}'}$}
			\AXC{$\async{\Psi}{\Delta}{\af{\phi_1}{e}, \Phi} \separator \constr{\Lambda, \used{e}}{\text{V}''}$}
			\LeftLabel{$[\llwith]$}
			\BIC{$\async{\Psi}{\Delta}{\af{\phi_1 \llwith \phi_2}{e}, \Phi} \separator \constr{\Lambda}{\text{V}', \text{V}''}$}	% capisco cosa fanno qui i constraint
			\DP}
			\\
			\SetCell[c=2]{c} {\footnotesize
			...
			\SetCell[c=2]{c} {\footnotesize
			\AXC{$\neg\isAsy{\phi}$}
			\AXC{$\async{\Psi}{\Delta, \af{\phi}{e}}{\Phi} \separator \constr{\Lambda}{\text{V}}$}
			\LeftLabel{$[R\!\Uparrow]$}
			\BIC{$\async{\Psi}{\Delta}{\af{\phi}{e}, \Phi} \separator \constr{\Lambda}{\text{V}}$}
			\DP
			}
		\end{tblr}
...
\begin{figure}[H]
	...
	\begin{subfigure}{\textwidth}
		...
		\begin{tblr}{ colspec = { cc } 
			...
			\LeftLabel{$[\llten]$}
			\UIC{$\focus{\Psi}{\Delta}{\text{af}(\phi_1 \llten \phi_2, e)} \separator \constr{\Lambda}{\text{V}''}$}	% capisco il movimento dei constraint
			\DP}
			\\ 
			{\footnotesize
			...
			\AXC{$\async{\Psi}{\Delta}{\af{\phi}{e}} \separator \constr{\Lambda}{\text{V}}$}
			\LeftLabel{$[R\!\Downarrow]$}
			\BIC{$\focus{\Psi}{\Delta}{\af{\phi}{e}} \separator \constr{\Lambda}{\text{V}}$}
			\DP
			}
		\end{tblr}
...
\begin{figure}[H]
	...
	\begin{subfigure}{\textwidth}
		...
		\begin{tblr}{ colspec = { cc }
			...
			\LeftLabel{$[I_1]$}
			\UIC{$\focus{\Psi}{\Delta, \af{\alpha}{e_2}}{\af{\llnot{\alpha}}{e_1}} \separator \constr{\Lambda}{\text{V}}$}
			\DP}
			&
			{\footnotesize
			\AXC{$\neg \isNegLit{\phi}$}
			...
			\LeftLabel{$[I_2]$}
			\UIC{$\focus{\Psi, \alpha}{\Delta}{\af{\llnot{\alpha}}{e_1}} \separator \constr{\Lambda}{\text{V}}$}
			\DP}
			&
			{\footnotesize
			\AXC{$\neg \isNegLit{\phi}$}
...
\begin{define}[Constraint evaluation]
...
\end{center}
	...
	\begin{itemize}
		\item[$\llten$:] 
			...
			\begin{align*}
				\focus{\Psi}{\Delta_L^X}{\text{af}(\phi_1, e)} & \separator \constr{\Lambda, \used{e}}{\text{V}'} \\
				\focus{\Psi}{\Delta_R^X}{\text{af}(\phi_2, e)}  & \separator \constr{\text{V}'}{\text{V}''}
			\end{align*}
			hold.
			Now the set of all the variables referenced in the conclusion must be a subset of the sets of the variables of the branches, since a branch can only add new variables.
			Furthermore, given how the variables are defined, $\Delta_L^X[\text{V}']$ and $\Delta_R^X[\text{V}'']$ must be disjoint.
			Given this we get that this equals to 
			$$
			\AXC{$\focus{\Psi}{\Delta_L^X[\text{V}'']}{\phi_1}$}
			\AXC{$\focus{\Psi}{\Delta_R^X[\text{V}'']}{\phi_2}$}
			\LeftLabel{$[\llten]$}
			\BIC{$\focus{\Psi}{\Delta_L^X[\text{V}''], \Delta_R^X[\text{V}'']}{\phi_1 \llten \phi_2}$}
			\DP
			$$
		\item[$\llbot$:] 
			$$
			...
			and
			$$
			\AXC{$\async{\Psi}{\Delta[\text{V}}{\Phi[\text{V}}$}
			\LeftLabel{$[\llbot]$}
			\UIC{$\async{\Psi}{\Delta[\text{V}}{\llbot, \Phi[\text{V}]}$}
			\DP
			$$
		%	\item[$\lltop$:]
		%		$$
		...
		\item[$\llpar$:]
			...
			$$ \async{\Psi}{\Delta}{\af{\phi_1}{e}, \af{\phi_2}{e}, \Phi} \separator \constr{\Lambda, \used{e}}{\text{V}}$$
			Hence there is a linear proof of 	
			$$
			\AXC{$\async{\Psi}{\Delta[\text{V}]}{\phi_1, \phi_2, \Phi[\text{V}}$}
			\LeftLabel{$[\llpar]$}
			\UIC{$\async{\Psi}{\Delta[\text{V}]}{\phi_1 \llpar \phi_2, \Phi[\text{V}]}$}
			\DP
			$$
			where $\Delta' = \Delta[\text{V}]$.
		\item[$\llplus_L$:]
			$$
			\AX$\focus{\Psi}{\Delta}{\af{\phi_1}{e}} \fCenter \separator \constr{\Lambda, \used{e}}{\text{V}}$
			\LeftLabel{$[\llplus_L]$}
			...
			$$
			\AXC{$\focus{\Psi}{\Delta[\text{V}]}{\phi_1}$}
			\LeftLabel{$[\llplus_L]$}
			\UIC{$\focus{\Psi}{\Delta[\text{V}]}{\phi_1 \llplus \phi_2}$}
			\DP
			$$
		\item[$\llplus_R$:]
			$$
			...
			$$
			\AXC{$\focus{\Psi}{\Delta[\text{V}]}{\phi_2}$}
			\LeftLabel{$[\llplus_R]$}
			\UIC{$\focus{\Psi}{\Delta[\text{V}]}{\phi_1 \llplus \phi_2}$}
			\DP
			$$
		\item[$\llwith$:]
			...
			% might need to fix
			$$
			\AXC{$\async{\Psi}{\Delta[\text{V}]}{\phi_1, \Phi[\text{V}]}$}
			\AXC{$\async{\Psi}{\Delta[\text{V}]}{\phi_2, \Phi[\text{V}]}$}
			\LeftLabel{$[\llwith]$}
			\BIC{$\async{\Psi}{\Delta[\text{V}]}{\phi_1 \llwith \phi_2, \Phi[\text{V}]}$}
			\DP
			$$
		\item[$\llbang{}$:]
			$$
		...
		\item[$\llwn{}$:]
			...
			$$
			$$
			\AXC{$\async{\phi, \Psi}{\Delta[\text{V}]}{\Phi[\text{V}]}$}
			\LeftLabel{$[\llwn{}]$}
			\UIC{$\async{\Psi}{\Delta[\text{V}]}{\llwn{\phi}, \Phi[\text{V}]}$}
			\DP
			$$
		\item[$R\!\Downarrow$:] This rule is trivial
			...
			$$
			\AXC{$\isAsy{\phi} \vee \isNegLit{\phi}$}
			\AXC{$\async{\Psi}{\Delta[\text{V}]}{\phi}$}
			\LeftLabel{$[R\!\Downarrow]$}
			\BIC{$\focus{\Psi}{\Delta[\text{V}]}{\phi}$}
			\DP
			$$
