...
\begin{proof}
	...
	\begin{itemize}
		\item[$\llten$:] 
			...
			\LeftLabel{$[\llten]$}
			\BinaryInfC{$\vdash \llwn{\Psi}, \llwn{\Psi}, \Delta', \Delta'', \phi_1 \llten \phi_2$}
			\LeftLabel{$[>\!*]$}
			\UnaryInfC{$\vdash \llwn{\Psi}, \Delta', \Delta'', \phi_1 \llten \phi_2$}
			\DisplayProof
			$$
		\item[$\llbot$:] 
		\item[$\lltop$:]
		\item[$\llpar$:]
			...
			\DisplayProof
			$$
			Assuming the hypotesis holds
			$$ \async{\Psi}{\Delta}{\af{\phi_1}{e}, \af{\phi_2}{e}, \Phi} \separator \constr{\Lambda, \used{e}}{\text{V}}$$
			We trivially can % recuperare
			$$
			\AxiomC{$\vdash \llwn{\Psi}, \Delta, \phi_1, \phi_2$}
			\LeftLabel{$[\llpar]$}
			\UnaryInfC{$\vdash \llwn{\Psi}, \Delta, \phi_1 \llpar \phi_2$}
			\DisplayProof
			$$
		\item[$\llplus_L$:]
			$$
